...
import { Game } from '../context/TeamContext'; // Import Game type

interface EditGameModalProps {
  isOpen: boolean;
  onClose: () => void;
  game: Game | null; // Pass the game object to edit
  onUpdateGame: (id: string, updates: Partial<Omit<Game, 'id'>>) => void; // Use update function
}
...
const EditGameModal: React.FC<EditGameModalProps> = ({ isOpen, onClose, game, onUpdateGame }) => {
  ...
  useEffect(() => {
    // Populate form when modal opens with game data
    if (isOpen && game) {
    ...
    // Reset form if modal closes or game is null
    if (!isOpen) {
        setOpponent('');
        setDate('');
        setTime('');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (game && opponent.trim() && date) {
      onUpdateGame(game.id, { // Call update function with game ID and updates
  ...
  if (!isOpen || !game) return null; // Don't render if not open or no game
